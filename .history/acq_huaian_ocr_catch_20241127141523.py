...
SAVE_PATH_BAK = r'/home/user/ship_data240/bak'
...
def post_process_ship_name(ocr_result, threshold: float = 0.625):
    ...
    char_map = {
        ...
        'G': '6', 'B': '8', '贷': '货', '莎': '苏', '劳': '苏', '花': '苏', '洞': '润',
        'F': '8', 'f': '8', 'M': '0', 'm': '0', 'N': '0', 'n': '0',
        'V': '0', 'v': '0', 'W': '0', 'w': '0', 'X': '0', 'x': '0',
        'Y': '0', 'y': '0', 'K': '0', 'k': '0', 'R': '0', 'r': '0',
        'T': '0', 't': '0', 'U': '0', 'u': '0', 'J': '0', 'j': '0',
    }
    ...
    for line in ocr_result:
        ...
        if len(text) >= 3:  # 船名通常大于4个字符
            if text[0].isdigit() and text[1].isdigit() and '\u4e00' <= text[-1] <= '\u9fff':
                text = text[::-1]
        # 检查省份前缀和类型后缀
        if any(text.startswith(prefix) for prefix in province_prefixes) and any(text.endswith(suffix) for suffix in type_suffixes):
...
def start_monitor(folder_path):
    ...
    while True:
        try:
            process_files(folder_path, yolomodel, ocr4model, udp_sender)
            time.sleep(10)
        except Exception as e:
            print(f"错误: {e}")
def start_monitor(folder_path):
...
def detect_and_track_ship(model, ocr4model, video_path, UseFile = False):
    ...
    if UseFile :
        ...
        rtsp_url = f"rtsp://admin:hkhk_11O@192.168.168.{Ip_num}"
        # if Ip_num == 113:
...
if __name__ == "__main__":
    ...
    video_path = "./test112.mp4"
    modelpath = "./best_boat12.pt"
