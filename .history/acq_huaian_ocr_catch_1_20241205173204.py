...
SAVE_PATH = r'/home/user/ship_data240'
date_folder = None
folder_path = None
SAVE_PATH_BAK = r'/home/user/ship_data240'

# def update_date_folder(new_date_folder,first_seen_date):
#     global date_folder, SAVE_PATH,folder_path,bak_folder
#     # 获取 SAVE_PATH 中的最后一个文件夹名
#     last_folder_name = os.path.basename(SAVE_PATH)
#     # 检查最后一个文件夹名是否为日期格式
#     try:
#         datetime.strptime(last_folder_name, '%Y%m%d')
#         is_date = True
#     except ValueError:
#         is_date = False
...
#         new_save_path = os.path.join(SAVE_PATH, current_date)
...
#         last_folder_name = current_date

#     # 检查最后一个文件夹名是否与 first_seen_date 相同
#     if last_folder_name != first_seen_date:
#         # 更新 SAVE_PATH
#         SAVE_PATH = os.path.join(os.path.dirname(SAVE_PATH), first_seen_date)
#         # 更新 date_folder
#         date_folder = os.path.join(SAVE_PATH, first_seen_date)
#         # 重新计算 bak_folder
#         bak_folder = os.path.join(SAVE_PATH, 'bak')
#         # 重新计算 folder_path
#         folder_path = SAVE_PATH
...
def savepic(frame, Ip_num, ObjectID, f=None):
    ...
    # SAVE_PATH = date_folder
    # update_date_folder(date_folder,first_seen_date)
    ...
    if not os.path.exists(frame_filename):
        print(f"文件保存失败: {frame_filename}")
    else:
        print(f"文件保存成功: {frame_filename}")
    if f:
...
def process_files(folder_path,yolomodel, ocr):
        ...
        if f.startswith('ID') and os.path.isfile(os.path.join(folder_path, f)):
            try:
                ...
                if id_num not in id_files:
                    id_files[id_num] = []
                    id_first_seen[id_num] = datetime.fromtimestamp(os.path.getmtime(os.path.join(folder_path, f))).strftime('%Y%m%d')
    ...
    if len(ids) > 0:
        ...
        if time.time() - newest_time > 40:  # 2分钟超时
    ...
    for id_num in process_ids:
        ...
        try :
            ...
            smb_path = "/run/user/1000/gvfs/smb-share:server=192.168.168.104,share=upload/ship"
            ...
            for f in image_paths:
                ...
                first_seen_date = id_first_seen[id_num]

                # 获取 SAVE_PATH 中的最后一个文件夹名
                last_folder_name = os.path.basename(SAVE_PATH_BAK)
                # 检查最后一个文件夹名是否为日期格式
                try:
                    datetime.strptime(last_folder_name, '%Y%m%d')
                    is_date = True
                ...
                if not is_date:
                    # 创建当前日期的子文件夹
                    current_date = datetime.now().strftime('%Y%m%d')
                    new_save_path = os.path.join(SAVE_PATH_BAK, current_date)
                    os.makedirs(new_save_path, exist_ok=True)
                    SAVE_PATH_BAK = new_save_path
                ...
                if last_folder_name != first_seen_date:
                    # 更新 SAVE_PATH
                    SAVE_PATH_BAK = os.path.join(os.path.dirname(SAVE_PATH_BAK), first_seen_date)
                # 更新 date_folder
                    dst_folder = os.path.join(SAVE_PATH_BAK, first_seen_date)
                else:
                    # 如果相同，不进行任何更改
                    dst_folder = SAVE_PATH_BAK
...
def detect_and_track_ship(model, ocrmodel, video_path, UseFile = False):
    ...
    while True:
        ...
        try:
            ...
            for detection in detections:
                ...
                #     object_id_last = object_id

                if (class_id == 0 or class_id == 1) and idx2 == 0:
                    label = f"ID{object_id+curr_id:03}"
                    # first_seen_date = datetime.now().strftime('%Y%m%d')  # 记录第一次出现的时间
                    ...
                    result = ocrmodel.ocr(frame, det=True, cls=True)
                    print(50*'*', file=f)
                    current_time = datetime.now().strftime("%Y-%m-%d %H:%M:%S.%f")[:-3]
...
if __name__ == "__main__":
    ...
    dev.GeneralSetting()
    dev.LoginDev(ip=b'192.168.168.240', username=b"admin", pwd=b"hkhk_119")
    dev.SetupAlarm()
    ...
    detect_and_track_ship(yolomodel, ocr4model, video_path, False) # True usefile
