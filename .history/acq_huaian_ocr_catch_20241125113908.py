...
def detect_and_track_ship(model, ocrmodel, video_path, UseFile = False):
    ...
    # 添加两个位置参数
    parser.add_argument('-i','--ip', type=int, default=112, help='')
    ...
    if UseFile :
        ...
        rtsp_url = f"rtsp://admin:hycz147258@192.168.168.{Ip_num}"
        # if Ip_num == 113:
    ...
    while True:
        ...
        try:
            ...
            for detection in detections:
                ...
                object_id = detection[4] if detection[4] is not None else 'N'  # 使用索引 4 获取ID（若存在）
...
if __name__ == "__main__":
    ...
    dev.GeneralSetting()
    dev.LoginDev(ip=b'192.168.168.114', username=b"admin", pwd=b"hycz147258")
    dev.SetupAlarm()
